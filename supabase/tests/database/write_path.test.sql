-- #7b regression test: companies.elo has exactly one writer (process_vote).
...
begin
  -- process_vote raises the winner's elo and logs exactly one votes row
  ...
  assert v_after > v_before, 'process_vote did not raise winner elo';
