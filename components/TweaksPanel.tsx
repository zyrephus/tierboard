...
export function TweakRadio({ label, value, options, onChange }: { label: string; value: string; options: RadioOption[]; onChange: (v: string) => void }) {
  ...
  const segAt = (clientX: number) => {
    ...
    const i = Math.floor(((clientX - r.left - 2) / inner) * options.length);
    return options[Math.max(0, Math.min(options.length - 1, i))].value;
  };
        ...
        {options.map((o, i) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio" aria-checked={on} data-on={on ? '1' : '0'}
                    title={o} style={{ background: o }} onClick={() => onChange(o)}>
